 ...
 *
 * It seeds the last-known position from REST, then joins the shipment socket
 * room and applies every `shipment-location-updated` broadcast. The room is
 * left and listeners are removed on cleanup, so opening/closing a drawer or
 * switching shipments never leaks subscriptions. Server authorization
 * (`assertCanRead`) still gates the join, so a customer can only watch their
 * own shipment.
 */
export function useShipmentLiveLocation(
  ...
  useEffect(() => {
    ...
    const onUpdate = (loc: LiveLocation) => {
      if (loc.shipmentId === shipmentId) setLocation(loc);
    };
