      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 sm:px-6 lg:px-8">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((item) => (
                  <li key={item.label}>
                    <Link href={item.href} className="text-sm font-medium text-[var(--text-on-dark-muted)] transition-colors hover:text-[var(--accent)]">
                      {item.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-[rgba(255,255,255,0.10)] pb-10 pt-6 md:flex-row">
          ...
          <div className="flex items-center gap-6">
            {legalLinks.map((item) => (
              <Link key={item.label} href={item.href} className="text-xs font-medium text-[var(--text-on-dark-muted)] transition-colors hover:text-[var(--accent)]">
                {item.label}
              </Link>
            ))}
          </div>
