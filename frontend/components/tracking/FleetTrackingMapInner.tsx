  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:42px;height:54px;">
        ...
        <svg width="42" height="54" viewBox="0 0 42 54" xmlns="http://www.w3.org/2000/svg" style="filter:drop-shadow(0 10px 14px rgba(58,46,18,.22));">
        ...
        <span style="position:absolute;left:50%;top:-2px;transform:translateX(-50%);max-width:70px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(58,46,18,.12);border-radius:999px;background:#fff;padding:2px 7px;font:700 10px/1.2 Inter,system-ui,sans-serif;color:var(--accent-strong);">${label}</span>
      </div>`,
    iconSize: [42, 54],
    iconAnchor: [21, 52],
    popupAnchor: [0, -50],
    ...
    >
      ...
      <Popup>
        <div className="min-w-40">
          <p className="text-sm font-bold text-[var(--accent-strong)]">#{marker.trackingId}</p>
          <p className="mt-1 text-xs font-semibold text-[var(--inactive)]">
            {marker.driverName}
          </p>
          <p className="mt-1 text-xs text-[var(--inactive)]">{marker.statusLabel}</p>
        </div>
      </Popup>
    </Marker>
  );
