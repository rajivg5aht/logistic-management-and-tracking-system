...
function FeaturedCategories() {
  const categories = [
    ...
    {
      icon: <HardDrive size={22} className="text-[#00E5FF]" />,
  ...
  return (
    <Section id="categories" className="bg-[#050816]">
      <div className="glow-orb glow-orb-sm" style={{ top: '50%', left: '25%', transform: 'translate(-50%, -50%)' }} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7">
        {categories.map((cat, idx) => (
          <Card key={idx} padding="p-10" className="flex flex-col justify-between min-w-[280px]" hover={true} glow={true}>
            <div>
              <div className="w-12 h-12 rounded-2xl bg-[#00E5FF]/10 flex items-center justify-center mb-6 border border-[#00E5FF]/20 shadow-[0_0_15px_rgba(0,229,255,0.08)]">
                {cat.icon}
              </div>
              <h3 className="heading-sm mb-2 group-hover:text-[#00E5FF] transition-colors">{cat.title}</h3>
              <p className="body-text-sm mb-6">{cat.desc}</p>
            </div>
            <a href={cat.link} className="inline-flex items-center gap-2 text-xs font-bold text-[#00E5FF] hover:text-white transition-colors group/btn">
...
function PopularProducts() {
  const products = [
    {
      ...
      rating: 5,
      svg: (
        <svg viewBox="0 0 200 120" className="w-full h-full text-[#00E5FF]" fill="none" stroke="currentColor" strokeWidth="1.5">
          <rect x="50" y="30" width="100" height="60" rx="10" stroke="rgba(255,255,255,0.08)" fill="#0B1220" />
          <circle cx="100" cy="60" r="16" stroke="rgba(0,229,255,0.2)" strokeWidth="1" fill="#050816" />
          <circle cx="100" cy="60" r="6" fill="#00E5FF" className="animate-pulse" />
    ...
    },
    {
      ...
      svg: (
        <svg viewBox="0 0 200 120" className="w-full h-full text-[#00E5FF]" fill="none" stroke="currentColor" strokeWidth="1.5">
          <circle cx="60" cy="60" r="8" fill="#0B1220" stroke="rgba(255,255,255,0.08)" />
          <circle cx="140" cy="60" r="8" fill="#0B1220" stroke="rgba(255,255,255,0.08)" />
          <circle cx="100" cy="35" r="8" fill="#0B1220" stroke="rgba(255,255,255,0.08)" />
          <circle cx="100" cy="85" r="8" fill="#050816" stroke="#00E5FF" />
          <path d="M68 60 h64 M100 43 v34" stroke="rgba(0,229,255,0.3)" />
          <path d="M100 35 L60 60 L100 85 L140 60 Z" stroke="#00E5FF" strokeWidth="1" strokeDasharray="3 3" />
        </svg>
      ),
    },
    {
      ...
      svg: (
        <svg viewBox="0 0 200 120" className="w-full h-full text-[#00E5FF]" fill="none" stroke="currentColor" strokeWidth="1.5">
          <rect x="60" y="35" width="80" height="50" rx="5" stroke="rgba(255,255,255,0.08)" fill="#0B1220" />
          <path d="M70 45 h40 M70 55 h60 M70 65 h30" stroke="rgba(255,255,255,0.2)" strokeWidth="2" strokeLinecap="round" />
          <rect x="110" y="65" width="20" height="12" rx="2" stroke="#00E5FF" fill="#050816" />
          <circle cx="120" cy="71" r="2" fill="#00E5FF" />
        </svg>
      ),
    },
    {
      ...
      svg: (
        <svg viewBox="0 0 200 120" className="w-full h-full text-[#00E5FF]" fill="none" stroke="currentColor" strokeWidth="1.5">
          <rect x="70" y="25" width="60" height="70" rx="6" stroke="rgba(255,255,255,0.08)" fill="#0B1220" />
          <rect x="78" y="33" width="44" height="30" rx="2" stroke="rgba(0,229,255,0.3)" fill="#050816" />
          <rect x="88" y="70" width="24" height="6" rx="1" fill="#00E5FF" />
          <circle cx="88" cy="84" r="3" fill="rgba(255,255,255,0.3)" />
          <circle cx="100" cy="84" r="3" fill="rgba(255,255,255,0.3)" />
          <circle cx="112" cy="84" r="3" fill="rgba(255,255,255,0.3)" />
        </svg>
      ),
    },
  ];
  ...
  return (
    <Section id="products" className="bg-[#050816]">
      <div className="glow-orb glow-orb-sm" style={{ bottom: 0, right: '25%', transform: 'translateX(50%)' }} />
      <div className="section-header">
        ...
        {products.map((prod, idx) => (
          <Card key={idx} padding="p-8 lg:p-10" className="flex flex-col justify-between min-w-[280px]" hover={true} glow={true}>
            <div className="bg-[#050816]/75 border border-white/[0.03] rounded-xl overflow-hidden aspect-[1.5] mb-5 flex items-center justify-center relative group-hover:border-[#00E5FF]/10 transition-colors">
              <div className="absolute top-3 left-3 px-2.5 py-1 rounded-md bg-[#0B1220]/80 border border-white/[0.04] text-[10px] font-bold tracking-wider text-white/40 uppercase">{prod.type}</div>
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div>
                <div className="flex gap-0.5 mb-2">
                  {Array.from({ length: prod.rating }).map((_, sIdx) => (
                    <Star key={sIdx} size={12} className="text-[#00E5FF] fill-[#00E5FF]" />
                  ))}
                </div>
                <h3 className="heading-sm mb-2 group-hover:text-[#00E5FF] transition-colors">{prod.title}</h3>
              ...
              <div className="pt-4 border-t border-white/[0.04] flex items-center justify-between mt-auto">
                <span className="text-lg font-bold text-white tracking-tight">{prod.price}</span>
